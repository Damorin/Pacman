...
public class Executor {
	...
	public static void main(String[] args) {
		...
		List<Controller> players = getAvailableInstances(Controller.class);
		//
		for (Controller<MOVE> player : players) {
			...
			exec.runExperiment(player, new Legacy(), numTrials);
		}

		// run a game in synchronous mode: game waits until controllers respond.
		// int delay=16;
		// boolean visual=true;
		// exec.runGame(new ArturasSikorskis(),new Legacy(),visual,delay);
	...
	public static <T> List<T> getAvailableInstances(Class<T> type) {
		...
		File dir = new File(url.getFile() + "/students");
